...
const MapFilter = ({ filterContext }) => {
  ...
  const renderFieldBasedOnType = option => {
    switch (option.type) {
      case 'RangeInput':
        return (
          <RangeInput
  ...
  const tableData = [{ id: 1, val: 'ex' }, { id: 2, val: 'ex2' }]

  const [tblData, setTblData] = useState(tableData)

  const { filterData, changeYear } = filterContext
  ...
  return (
    ...
    <div>
      ...
      <div>
        <Accordion>
          {Object.values(filterData).map(data => {
            return (
              <AccordionPanel key={data.name} label={data.name}>
                <Box pad="xsmall" background="light-2">
          ...
          {/* <AccordionPanel label="Boundaries">
            <Box pad="xsmall" background="light-2">
              ...
              <RangeInput
                ...
                max={2010}
                step={1}
                value={filterContext.filterData.boundary.options[0].value}
                onChange={e => filterContext.changeYear(e.target.value)}
              />
              <Text>Area</Text>
              <Select
                options={['National', 'States', 'PUMAs/Country groups']}
